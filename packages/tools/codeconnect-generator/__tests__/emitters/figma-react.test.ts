  ...
  describe('emit', () => {
    describe('positive cases', () => {
      ...
      it('should generate JSX example with className and props spread', () => {
        const model = createMockComponentModel({
          className: 'Accordion',
          filePath: '/src/accordion.component.ts',
          componentDir: '/src',
        });

        const result = emitter.emit({ model, options: createMockEmitterOptions() });

        expect(result.content).toContain('<Accordion {...props} />');
      });
    ...
    describe('edge cases', () => {
      ...
      it('should prefix relative import paths that do not start with a dot', () => {
        const relativeSpy = jest.spyOn(path.posix, 'relative').mockReturnValue('dist/react');
        const model = createMockComponentModel({
          className: 'Button',
          tagName: 'mdc-button',
          filePath: '/repo/component/button.component.ts',
