...
**IMPORTANT:** The Focus Ring is following the HTML standards by using `focus-visible`, which will make the visible FocusRing appear around the button
for example only when tabbing to it, not when clicking (`document.activeElement` will be the button in both cases). 
