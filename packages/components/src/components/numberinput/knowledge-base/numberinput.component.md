...
summary:
  ...
  numeric text field with increment and decrement spinner buttons and native
...
| Option                                          | Intent                                                                                                                                                                                                                                                |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `min` / `max`                                   | Bound the accepted range. Set both when a valid range exists so the browser can enforce and announce it; the spinner buttons and arrow keys clamp to these bounds.                                                                                    |
| `step` (default `1`)                            | The amount each spinner button click or arrow-key press changes the value, and the granularity used for step-mismatch validation.                                                                                                                     |
| `step="any"`                                    | Accepts decimal values without step-mismatch validation. The spinner buttons and arrow keys then move by 1 and clamp, matching native spin buttons. For values that must follow a fixed increment, use a numeric `step` (for example `0.01`) instead. |
| `hide-spinner-buttons` (default `false`)        | Hides the increment/decrement spinner buttons and renders the field as a plain numeric input (with the clear button when `trailing-button` is set).                                                                                                   |
| `increment-aria-label` / `decrement-aria-label` | Accessible names for the spinner buttons. Empty by default; set both so the buttons are announced.                                                                                                                                                    |
...
- **Typed out-of-range values aren't clamped** — typing a value beyond
  ...
  `rangeOverflow`/`rangeUnderflow`; only the spinner buttons and arrow keys
  clamp. Read the field's validity to handle the invalid state.
- **Boundary steps are silent** — a spinner button click or arrow-key press at
  `min`/`max` leaves the value unchanged and emits no `input`/`change`. Don't
  rely on an event to detect a no-op step.
- **`step="any"` steps by 1** — with `step="any"` the spinner buttons and arrow
  keys add or subtract 1 rather than a fractional amount. Set a numeric `step`
  if you need fractional stepping.
...
- **Spinner buttons aren't in the tab order** — the increment/decrement buttons
  have `tabindex="-1"` by design and cannot receive keyboard focus. Keyboard
  users perform the equivalent adjustment from the input with
  `ArrowUp`/`ArrowDown`.

### Notes

- Built on `mdc-input`; it inherits the label, help text, toggletip, and
  validation surface, and renders spinner buttons using `mdc-button` with
...
press `ArrowUp`/`ArrowDown` to increment or decrement by `step`. The spinner
buttons are kept out of the tab order so keyboard users adjust the value with
the arrow keys, and non-numeric characters are prevented on input.

#### Internal ARIA managed by the component

| Element                                 | Attribute    | Value                                                                                                         |
| --------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------- |
| Increment spinner button (`mdc-button`) | `aria-label` | value of `increment-aria-label`                                                                               |
| Decrement spinner button (`mdc-button`) | `aria-label` | value of `decrement-aria-label`                                                                               |
| Spinner buttons                         | `tabindex`   | `-1` — excluded from the tab order; keyboard users change the value from the input with `ArrowUp`/`ArrowDown` |
...
- Set `increment-aria-label` and `decrement-aria-label` — they are empty by
  default, leaving the spinner buttons unnamed for screen-reader users.
