...
- Component folder: [`packages/components/src/components/cardcheckbox/`](../../components/src/components/cardcheckbox/)
- Built metadata references: `components/cardcheckbox/cardcheckbox.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/cardcheckbox';
```

```html
<mdc-cardcheckbox></mdc-cardcheckbox>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { CardCheckbox } from '@momentum-design/components/react';
```
