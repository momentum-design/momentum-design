...
- Component folder: [`packages/components/src/components/linkbutton/`](../../components/src/components/linkbutton/)
- Built metadata references: `components/linkbutton/linkbutton.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/linkbutton';
```

```html
<mdc-linkbutton></mdc-linkbutton>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-linkbutton--docs) or Canvas view.
