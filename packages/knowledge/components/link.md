...
- Component folder: [`packages/components/src/components/link/`](../../components/src/components/link/)
- Built metadata references: `components/link/link.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/link';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-link--docs) or Canvas view.
