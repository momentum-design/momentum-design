...
- Component folder: [`packages/components/src/components/linksimple/`](../../components/src/components/linksimple/)
- Built metadata references: `components/linksimple/linksimple.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/linksimple';
```

```html
<mdc-linksimple></mdc-linksimple>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { Linksimple } from '@momentum-design/components/react';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-linksimple--docs) or Canvas view.
