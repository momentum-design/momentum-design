...
- Component folder: [`packages/components/src/components/avatarbutton/`](../../components/src/components/avatarbutton/)
- Built metadata references: `components/avatarbutton/avatarbutton.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/avatarbutton';
```

```html
<mdc-avatarbutton></mdc-avatarbutton>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-avatarbutton--docs) or Canvas view.
...
See [Avatar](./avatar.md) documentation.
