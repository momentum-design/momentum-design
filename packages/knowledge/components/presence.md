...
- Component folder: [`packages/components/src/components/presence/`](../../components/src/components/presence/)
- Built metadata references: `components/presence/presence.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/presence';
```

```html
<mdc-presence></mdc-presence>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-presence--docs) or Canvas view.
