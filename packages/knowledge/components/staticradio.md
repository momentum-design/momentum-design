...
- Component folder: [`packages/components/src/components/staticradio/`](../../components/src/components/staticradio/)
- Built metadata references: `components/staticradio/staticradio.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/staticradio';
```

```html
<mdc-staticradio></mdc-staticradio>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-decorator-staticradio--docs) or Canvas view.
