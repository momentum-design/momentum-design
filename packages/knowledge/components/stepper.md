...
- Component folder: [`packages/components/src/components/stepper/`](../../components/src/components/stepper/)
- Built metadata references: `components/stepper/stepper.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/stepper';
```

```html
<mdc-stepper></mdc-stepper>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { Stepper } from '@momentum-design/components/react';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-stepper-stepper--docs) or Canvas view.
