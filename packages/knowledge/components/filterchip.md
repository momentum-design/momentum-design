...
- Component folder: [`packages/components/src/components/filterchip/`](../../components/src/components/filterchip/)
- Built metadata references: `components/filterchip/filterchip.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/filterchip';
```

```html
<mdc-filterchip></mdc-filterchip>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-chip-filter--docs) or Canvas view.
