# VirtualizedList (`mdc-virtualizedlist`)
...
- Component folder: [`packages/components/src/components/virtualizedlist/`](../../components/src/components/virtualizedlist/)
- Built metadata references: `components/virtualizedlist/virtualizedlist.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/virtualizedlist';
```

```html
<mdc-virtualizedlist></mdc-virtualizedlist>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { VirtualizedList } from '@momentum-design/components/react';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-virtualizedlist--docs) or Canvas view.
