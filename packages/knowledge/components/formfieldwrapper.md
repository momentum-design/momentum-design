# FormfieldWrapper (`mdc-formfieldwrapper`)
...
- Component folder: [`packages/components/src/components/formfieldwrapper/`](../../components/src/components/formfieldwrapper/)
- Built metadata references: `components/formfieldwrapper/formfieldwrapper.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/formfieldwrapper';
```

```html
<mdc-formfieldwrapper></mdc-formfieldwrapper>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { FormfieldWrapper } from '@momentum-design/components/react';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/internal-formfieldwrapper--docs) or Canvas view.
