...
- Component folder: [`packages/components/src/components/buttongroup/`](../../components/src/components/buttongroup/)
- Built metadata references: `components/buttongroup/buttongroup.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/buttongroup';
```

```html
<mdc-buttongroup></mdc-buttongroup>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { ButtonGroup } from '@momentum-design/components/react';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-buttongroup--docs) or Canvas view.
