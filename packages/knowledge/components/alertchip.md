...
- Component folder: [`packages/components/src/components/alertchip/`](../../components/src/components/alertchip/)
- Built metadata references: `components/alertchip/alertchip.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/alertchip';
```

```html
<mdc-alertchip></mdc-alertchip>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-chip-alert--docs) or Canvas view.
