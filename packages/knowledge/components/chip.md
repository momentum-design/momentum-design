...
For best results, we recommend limiting the maximum length of the label text to 20 characters,
...
- Component folder: [`packages/components/src/components/chip/`](../../components/src/components/chip/)
- Built metadata references: `components/chip/chip.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/chip';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-chip-label--docs) or Canvas view.
