...
- Component folder: [`packages/components/src/components/optgroup/`](../../components/src/components/optgroup/)
- Built metadata references: `components/optgroup/optgroup.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/optgroup';
```

```html
<mdc-optgroup></mdc-optgroup>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { OptGroup } from '@momentum-design/components/react';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-select-optgroup--docs) or Canvas view.
