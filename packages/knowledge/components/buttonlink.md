...
- Component folder: [`packages/components/src/components/buttonlink/`](../../components/src/components/buttonlink/)
- Built metadata references: `components/buttonlink/buttonlink.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/buttonlink';
```

```html
<mdc-buttonlink></mdc-buttonlink>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-buttonlink--docs) or Canvas view.
