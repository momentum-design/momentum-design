...
- Component folder: [`packages/components/src/components/password/`](../../components/src/components/password/)
- Built metadata references: `components/password/password.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/password';
```

```html
<mdc-password></mdc-password>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-password--docs) or Canvas view.
