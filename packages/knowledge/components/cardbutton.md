...
- Component folder: [`packages/components/src/components/cardbutton/`](../../components/src/components/cardbutton/)
- Built metadata references: `components/cardbutton/cardbutton.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/cardbutton';
```

```html
<mdc-cardbutton></mdc-cardbutton>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { CardButton } from '@momentum-design/components/react';
```
