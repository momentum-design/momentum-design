...
- Component folder: [`packages/components/src/components/searchfield/`](../../components/src/components/searchfield/)
- Built metadata references: `components/searchfield/searchfield.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/searchfield';
```

```html
<mdc-searchfield></mdc-searchfield>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-searchfield--docs) or Canvas view.
