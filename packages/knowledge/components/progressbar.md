...
- Component folder: [`packages/components/src/components/progressbar/`](../../components/src/components/progressbar/)
- Built metadata references: `components/progressbar/progressbar.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/progressbar';
```

```html
<mdc-progressbar></mdc-progressbar>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-progressbar--docs) or Canvas view.
