...
- Component folder: [`packages/components/src/components/toggle/`](../../components/src/components/toggle/)
- Built metadata references: `components/toggle/toggle.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/toggle';
```

```html
<mdc-toggle></mdc-toggle>
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-toggle--docs) or Canvas view.
