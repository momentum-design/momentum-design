...
- Component folder: [`packages/components/src/components/animation/`](../../components/src/components/animation/)
- Built metadata references: `components/animation/animation.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/animation';
```

```html
<mdc-animation></mdc-animation>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { Animation } from '@momentum-design/components/react';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-animation--docs) or Canvas view.
