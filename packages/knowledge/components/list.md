...
- Component folder: [`packages/components/src/components/list/`](../../components/src/components/list/)
- Built metadata references: `components/list/list.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/list';
```
...
```javascript
import { List } from '@momentum-design/components/react';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-list--docs) or Canvas view.
