...
- Component folder: [`packages/components/src/components/listbox/`](../../components/src/components/listbox/)
- Built metadata references: `components/listbox/listbox.component.js` (from Custom Elements Manifest)
...
```javascript
import '@momentum-design/components/components/listbox';
```

```html
<mdc-listbox></mdc-listbox>
```

**React** wrapper (from `@lit/react` codegen):

```javascript
import { ListBox } from '@momentum-design/components/react';
```
...
Project Storybook enables the **Accessibility** addon with axe rules for **WCAG 2.x / 2.2 AA** and **best-practice** (see [`preview.jsx`](../../components/config/storybook/preview.jsx), `parameters.a11y`). Run checks from the [Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-listbox--docs) or Canvas view.
