...
- Executing `yarn components dev` in the project root will start the development server for the Components package, allowing you to view and test your components in a local environment.
- The `packages/components` directory contains the source code for all components, organized in subdirectories by component name (e.g., `packages/components/src/components/button`, `packages/components/src/components/card`, etc.).
...
- Follow the coding conventions outlined in #packages/components/conventions/\*_/_.md
...
- When running tests locally, ignore test failures if a browser is not installed or available in the environment. This allows developers to run tests without needing to set up all browsers, while still ensuring that tests will run in CI environments where browsers are available.
